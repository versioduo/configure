...
class V2MIDI {
  ...
  // Outgoing messages.
  sendNote(device, channel, note, velocity) {
    device.send([V2MIDI.Status.noteOn | channel, note, velocity]);
  }

  sendNoteOff(device, channel, note, velocity) {
    if (velocity == null)
      velocity = 64;
    device.send([V2MIDI.Status.noteOff | channel, note, velocity]);
  }

  sendControlChange(device, channel, controller, value) {
    device.send([V2MIDI.Status.controlChange | channel, controller, value]);
  }

  sendProgramChange(device, channel, value) {
    device.send([V2MIDI.Status.programChange | channel, value]);
  }

  sendAftertouchChannel(device, channel, value) {
    device.send([V2MIDI.Status.aftertouchChannel | channel, value]);
  }

  sendPitchBend(device, channel, value) {
    const bits = value + 8192;
    const msb = (bits >> 7) & 0x7f;
    const lsb = bits & 0x7f;
    device.send([V2MIDI.Status.pitchBend | channel, lsb, msb]);
  }

  sendSystemReset(device) {
    device.send([V2MIDI.Status.systemReset]);
  }

  sendSystemExclusive(device, message) {
